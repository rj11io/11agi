...
const fixtureRoot = mkdtempSync(join(tmpdir(), "11agi-benchmarks-project-"))
...
try {
  ...
  assert.match(markdown, /^# AI benchmarks and analysis: Project Report\n\n_powered by \[11agi-benchmarks-project\]\(https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-project\)\._\n\n/)
  ...
  assert.ok(markdown.endsWith("_AI benchmarks and analysis by [11agi-benchmarks-project](https://agi.rj11.io/skills/11agi-benchmarks-project)._\n"))
  ...
  assert.match(html, /<h1>AI benchmarks and analysis: Project Report <span class="powered-by"><a href="https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-project" target="_blank" rel="noopener noreferrer">powered by 11agi-benchmarks-project<\/a><\/span><\/h1>/)
  assert.equal((html.match(/powered by 11agi-benchmarks-project/g) ?? []).length, 1)
  ...
  assert.match(html, /<a href="https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-project" target="_blank" rel="noopener noreferrer">11agi-benchmarks-project<\/a>/)
  ...
  assert.match(unmatchedHtml, /<a href="https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-pricing-update" target="_blank" rel="noopener noreferrer">11agi-benchmarks-pricing-update<\/a>/)
  ...
  const reportsRoot = join(resolvedThreadRoot, "11agi-benchmarks-project-reports")
  ...
  assert.match(basename(defaultReportDir), /^11agi-benchmarks-project-reports-\d{4}-\d{2}-\d{2}T/)
  ...
  assert.match(basename(defaultSummary.markdownReport), /^11agi-benchmarks-project-\d{4}-\d{2}-\d{2}T.*\.md$/)
  ...
  writeFileSync(join(dirname(coworkSessionDir), "remote-session-spaces.json"), JSON.stringify({ entries: [{ sessionId: "remote-project-cowork", folders: [coworkProject] }] }))
  ...
  assert.equal(genericDataset.generator.skill, "11agi-benchmarks-project")
  ...
  const genericHtml = readFileSync(genericSummary.htmlReport, "utf8")
  assert.match(genericHtml, /<html lang="en" class="dark">/)
  assert.match(genericHtml, /id="theme-toggle"/)
  assert.match(genericHtml, /data-row-id="t1-r0"/)
  assert.match(genericHtml, /class="col-resize"/)
  assert.match(genericHtml, /font-variant-numeric: tabular-nums/)
  assert.equal((genericHtml.match(/data:font\/woff2;base64,/g) || []).length, 2)
  assert.equal((genericHtml.match(/unicode-range: U\+/g) || []).length, 2)
  assert.match(genericHtml, /replaceChildren/)
  assert.match(genericHtml, /--accent-surface/)
  ...
  const stripGenerated = (text) => text.replaceAll(/\d{4}-\d{2}-\d{2}T[\d:.]+Z/g, "<timestamp>")
  assert.equal(stripGenerated(readFileSync(rerenderReport, "utf8")), stripGenerated(genericMarkdown))
  assert.equal(stripGenerated(readFileSync(rerenderSummary.htmlReport, "utf8")), stripGenerated(genericHtml))
