...
const fixtureRoot = mkdtempSync(join(tmpdir(), "11agi-benchmarks-machine-"))
...
try {
  ...
  assert.equal(summaryDataset.generator.skill, "11agi-benchmarks-machine")
  ...
  assert.match(markdown, /^# AI benchmarks and analysis: Machine Report\n\n_powered by \[11agi-benchmarks-machine\]\(https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-machine\)\._\n\n/)
  ...
  assert.match(markdown, /\| Workspace \| Cost \| Input \| Cached \| Input cost \| Output \| Output cost \| Tokens \| Cost \/ 1M tokens \| Threads \| Cost \/ thread \| Active time \| Cost \/ active hour \| Wall time \| Cost \/ wall hour \| Output TPS \| Active time \/ response \| Sub-agent runs \| Priced \| Unpriced \|/)
  assert.match(markdown, /\| Thread \| Source \| Surface \/ billing \| Workspace \| Sub-agents \| Provider \/ model \/ effort \| Attributed at \| Input \| Cached \| Output \| Tokens \| Selected cost \| Active time \| Cost \/ active hour \| Wall time \| Cost \/ wall hour \| Output TPS \| Active time \/ response \| Harness reported \| Method \|/)
  ...
  assert.ok(markdown.endsWith("_AI benchmarks and analysis by [11agi-benchmarks-machine](https://agi.rj11.io/skills/11agi-benchmarks-machine)._\n"))
  ...
  assert.match(html, /<h1>AI benchmarks and analysis: Machine Report <span class="powered-by"><a href="https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-machine" target="_blank" rel="noopener noreferrer">powered by 11agi-benchmarks-machine<\/a><\/span><\/h1>/)
  assert.equal((html.match(/powered by 11agi-benchmarks-machine/g) ?? []).length, 1)
  ...
  assert.match(html, /<a href="https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-machine" target="_blank" rel="noopener noreferrer">11agi-benchmarks-machine<\/a>/)
  ...
  const reportsRoot = join(fakeDesktop, "11agi-benchmarks-machine-reports")
  assert.equal(dirname(defaultSummary.outputDirectory), reportsRoot)
  assert.match(basename(defaultSummary.outputDirectory), /^11agi-benchmarks-machine-reports-\d{4}-\d{2}-\d{2}T/)
  ...
  assert.match(basename(defaultSummary.markdownReport), /^11agi-benchmarks-machine-\d{4}-\d{2}-\d{2}T.*\.md$/)
  ...
  assert.match(coworkMarkdown, /\| claude-cowork \| claude \| cowork-sessions \|/)
  ...
  assert.match(legacyMarkdown, /\| Pricing catalog \| bundled default \(version 3, updated 2026-08-07\) \|/)
  ...
  assert.match(legacyMarkdown, /Known-cost totals exclude the models above\. Run \[11agi-benchmarks-pricing-update\]\(https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-pricing-update\)/)
  assert.match(legacyHtml, /<a href="https:\/\/agi\.rj11\.io\/skills\/11agi-benchmarks-pricing-update" target="_blank" rel="noopener noreferrer">11agi-benchmarks-pricing-update<\/a>/)
