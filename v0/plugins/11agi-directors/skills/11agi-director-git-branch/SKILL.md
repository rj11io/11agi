...
3. Commit the work to the task branch in focused commits. Branch commits are part of this workflow; pushing the branch to open the PR is too. Merging is not.
4. Write every commit message against the [Conventional Commits 1.0.0 specification](https://www.conventionalcommits.org/en/v1.0.0/) and any compatible repository rules:

   ```text
   type(optional scope): short description in the imperative
   ```

   Read the staged diff with `git diff --cached` and the recent subjects with `git log --format=%s -n 20` before choosing. Use `feat` for a new feature, `fix` for a bug fix, and otherwise a precise type the repository already uses — `docs`, `test`, `refactor`, `perf`, `build`, `ci`, or `chore`. Reuse an established scope rather than inventing one. Mark a breaking change with `!` before the colon and a `BREAKING CHANGE:` footer explaining what consumers must change. Never invent an issue reference, a reviewer, or a breaking change.

   Check the repository for a commit-message gate — commitlint, semantic-release, or contribution instructions — and follow it. When the repository squash-merges, the pull-request title becomes the commit subject, so it must satisfy the same rules.
