...
Native counters do not guarantee an invoice-equivalent price. Harness-reported cost is retained, while locally derived cost uses this skill's bundled pricing catalog. Gemini CLI use may be free, subscription-backed, or API-billed, so unmatched Google models remain visibly unpriced until `11agi-benchmarks-pricing-update` adds a provider-verified rate; consult [Gemini API pricing](https://ai.google.dev/gemini-api/docs/pricing).
